def setBuildEnvironment() {
    ...
    try {
        if ( currentBuild.number > 1 ) {
            ...
            gitDiff = sh (script: "git log ${oldGitShortCommitId}..${gitShortCommitId} --oneline --graph --decorate --no-merges --pretty='format:%C(auto)%h %ar  %C(Cyan)%s%C(reset) %cn'", returnStdout: true).trim()
