...
if ( STANDALONE_MODE != "true" ) {
    ...
    int max = 16
    int step = 1
    int startValue = worker_node_cpu_cores.toInteger()
    int minWarning = 2
    int valueDisplayConversion = 1
    def rangeUnit = "Cores"
    def warningText = "Warning. Allocating less than 2 cores of CPU to the worker nodes may not give you much room to play with"
    def infoText = "Determines the amount of CPU cores that will be allocated to the KX-Worker VM(s)"
    def paramIcon = "/userContent/icons/memory.svg"
    def paramShortTitle = "CPU Cores"
    def variablesSuffix = "worker_admin_node_cpu_cores"
    ...
    try {
        ...
        println "Something went wrong in the HTML return block (config_vm_properties_worker_node_cpu_cores): ${e}"
    }
