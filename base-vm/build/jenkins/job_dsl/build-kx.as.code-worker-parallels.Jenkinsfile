...
def jobProperties
Item currentJob = Jenkins.instance.getItemByFullName('Parallels/GeneratedJobs/01_Build_KX.AS.CODE_Worker')
if (currentJob) {
...
pipelineJob('Parallels/GeneratedJobs/01_Build_KX.AS.CODE_Worker') {
    definition {
      cps {
        script(readFileFromWorkspace('base-vm/build/jenkins/pipelines/build-kx.as.code-worker-parallels.Jenkinsfile'))
        sandbox()
      }
      queue("Parallels/GeneratedJobs/01_Build_KX.AS.CODE_Worker")
      if (jobProperties) {
      configure { root ->
