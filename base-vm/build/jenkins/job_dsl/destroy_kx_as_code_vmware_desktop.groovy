...
def jobProperties
Item currentJob = Jenkins.instance.getItemByFullName('VMWare_Workstation/GeneratedJobs/05_Destroy_KX.AS.CODE')
if (currentJob) {
...
pipelineJob('VMWare_Workstation/GeneratedJobs/05_Destroy_KX.AS.CODE') {
    definition {
      cps {
        script(readFileFromWorkspace('base-vm/build/jenkins/pipelines/destroy-kx.as.code-vmware-desktop.Jenkinsfile'))
        sandbox()
      }
      queue("VMWare_Workstation/GeneratedJobs/05_Destroy_KX.AS.CODE")
      if (jobProperties) {
      configure { root ->
