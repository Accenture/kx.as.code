...
def jobProperties
Item currentJob = Jenkins.instance.getItemByFullName('VirtualBox/GeneratedJobs/01_Build_KX.AS.CODE_Worker')
if (currentJob) {
...
pipelineJob('VirtualBox/GeneratedJobs/01_Build_KX.AS.CODE_Worker') {
    definition {
      cps {
        script(readFileFromWorkspace('base-vm/build/jenkins/pipelines/build-kx.as.code-worker-virtualbox.Jenkinsfile'))
        sandbox()
      }
      queue("VirtualBox/GeneratedJobs/01_Build_KX.AS.CODE_Worker")
      if (jobProperties) {
      configure { root ->
