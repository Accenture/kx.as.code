...
variable "iso_checksum" {
  type    = string
  default = "d32d2c63350a932dc0d9d45665985b41413f9e01efc0eacbea981d435f553d3d"
}
...
source "qemu" "kx-main-qemu" {
  # https://www.debian.org/releases/stable/arm64/apbs02.en.html#preseed-auto
  # https://hands.com/d-i/
  boot_command      = [
    "e<wait>",
    "<down>",
    ...
    "    linux /install.a64/vmlinuz ",
      ...
      "auto ",
      "preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/mac/${ var.preseed_path} ",
      "keyboard-configuration/xkb-keymap=us ",
      "netcfg/disable_dhcp=true ",
      "netcfg/get_ipaddress=10.0.2.15 ",
      "netcfg/get_netmask=255.255.255.0 ",
      "netcfg/get_gateway=10.0.2.2 ",
  ...
  iso_checksum      = "${ var.iso_checksum_type }:${ var.iso_checksum}"
  iso_urls          = [ "${ var.iso_path }/${ var.iso_file }", var.iso_url ]
  output_directory  = "../../../output-main/qemu-${ local.version }"
  shutdown_command  = var.shutdown_command
  ssh_password      = "vagrant"
  ssh_port          = 22
  ssh_timeout       = "3600s"
...
build {
  ...
  post-processor "vagrant" {
    keep_input_artifact  = true
    include              = ["../../../boxes/qemu-${ local.version }/info.json", "kx-main-${ local.version }_manifest.json"]
    only                 = ["qemu.kx-main-qemu"]
    output               = "../../../boxes/qemu-${ local.version }/kx-main-${ local.version }.box"
    vagrantfile_template = "../../../boxes/kx.as.code-main-qemu.Vagrantfile"
  }
  post-processor "shell-local" {
    environment_vars = ["VM_VERSION=${ local.version }", "VM_NAME=kx-main"]
    execute_command  = ["bash", "-c", "{{ .Vars }} {{ .Script }}"]
  ...
  post-processor "shell-local" {
    ...
    execute_command  = ["bash", "-c", "{{ .Vars }} {{ .Script }}"]
    only_on          = ["darwin", "linux"]
    script           = "../../../scripts/post-processing/vagrant_cloud_upload.sh"
  }
}
