...
const WorkspaceTab = ({ wsName, activeTab, setActiveTab }) => {
    ...
    return (
        <div onDoubleClick={handleDoubleClick} onClick={handleClick} className={`${activeTab === wsName ? "bg-ghBlack4 p-1 pl-3 hover:bg-ghBlack4 text-sm flex items-center justify-center border-t-[1px] border-kxBlue border-l-[1px] border-l-white/15" : "bg-ghBlack3 p-1 pl-3 hover:bg-ghBlack4 text-sm flex items-center justify-center border-t-[1px] border-ghBlack3 border-l-[1px] border-l-white/15"} hover:cursor-pointer`}>
            <div className="flex">
                <input readOnly={!isEditable} onDoubleClick={handleDoubleClick} onBlur={() => { setIsEditable(false) }} ref={inputRef} type="text" value={wsName} className={`pl-1 w-[120px] bg-transparent flex focus:outline-none border border-dotted ${isEditable ? 'border-white/50' : "border-transparent"} hover:cursor-pointer`} />
            </div>
