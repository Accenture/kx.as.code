...
Below an example metadata.json file for each installation method.
...
The helm installation process is executed via the following [script](https://github.com/Accenture/kx.as.code/blob/main/auto-setup/functions/autoSetupScriptInstall.sh){:target="\_blank"}.
...
| 	environment_variables	                                                     | Any variables need during the installation process. These will also be available during the installation process for mustache and environmernt variable substitutions                                                                                                                                             | 		                                                          |
| 	environment_variables.operatorVersion	                                     | Just an example environment variable                                                                                                                                                                                                                                                                              | 	`4.4.28`	                                                    |
...
| 	pre_install_scripts[0]	                                                    | Script to execute before the main installation process starts. TRhis is less important for the script based installation method, but more forHelm and ArgoCD. That said, it's still good to use the option here, to make the installation process more readable, rather than having everything in one long script | 	`createSecrets.sh`	                                          |
...
As everything else is the same, the JSON example will be complete, but only the ones not already described above will be described in more detail in the table below.

The example is based on the installation process defined ArgoCD. See here the [full solution](https://github.com/Accenture/kx.as.code/tree/main/auto-setup/cicd/argocd){:target="\_blank"}.
...
| 	Path Name                                                                | 	Description                                                                                                                                                                                                                                             | Example	                                                             |
|---------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------|
| 	minimum_resources	                                                       | Currently not used                                                                                                                                                                                                                                       | 		                                                                   |
| 	minimum_resources.cpu	                                                   | Currently not used                                                                                                                                                                                                                                       | 	`1000`	                                                               |
| 	minimum_resources.memory	                                                | Currently not used                                                                                                                                                                                                                                       | 	`3000`	                                                               |
...
| 	helm_params.set_key_values[1]	                                           | Another example of a set_key_value for ArgoCD                                                                                                                                                                                                            | 	installCRDs=false	                                                  |
| 	helm_params.set_key_values[2]	                                           | Another example of a set_key_value for ArgoCD                                                                                                                                                                                                            | 	configs.secret<br/>.argocdServerAdminPassword=<br/>'{{argoCdAdminPassword}}'	 |
| 	helm_params.set_key_values[3]	                                           | Another example of a set_key_value for ArgoCD                                                                                                                                                                                                            | 	controller.clusterAdminAccess<br/>.enabled=true	                         |
| 	helm_params.set_key_values[4]	                                           | Another example of a set_key_value for ArgoCD                                                                                                                                                                                                            | 	server.clusterAdminAccess<br/>.enabled=true	                             |
| 	helm_params.set_key_values[5]	                                           | Another example of a set_key_value for ArgoCD                                                                                                                                                                                                            | 	server.extraArgs[0]=--insecure	                                     |
...
|----------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------|
| 	$.installation_type	            | ----                                                                                                                                                                                                                                                                | 	argocd	                                   |
| 	$.retry	                        | `true`, `false` or `skip`. The default is `true`. If set to `false`, then the install will not be retried on error. If set to skip, then the component will merely be skipped on error and will not block the other components waiting in the queue to be installed | 	true	                                     |
| 	$.argocd_params	                | The parameters required for installing a solution according to ArgoCD GitOps.                                                                                                                                                                                       | 		                                         |
