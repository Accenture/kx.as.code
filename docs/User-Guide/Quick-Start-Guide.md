...
!!! note "You can ignore the Builder Config Panel at the bottom of the first tab, as the latest images are already available on Vagrant Cloud and will be downloaded automatically".
...
!!! warning  "Starting KX.AS.CODE with lower host system resources"
    If you have less than 16GB ram and less than 8 cores on your host, you should choose either the _Lite_ or _Minimal_ startup mode.
...
??? info Additional Toggles
    - **Enable Standalone Mode** - this sets number of main nodes to 1 and worker nodes to 0, and disables no longer needed network storage. It also sets Allow Workloads On Kubernetes Master to true, else otherwise it would not be possible to deploy anything.
...
!!! warning "Do not over allocate. The performance worsens if your host is starved of resources. At a minimum, leave at least 4 threads and 8gb ram to your host, so for a host with 32GB RAM and an Octa processor, you should allocate maximum, 12 of the 16 threads, and 24GB of the 32GB RAM available."
    | Host | RAM Allopcation | CPU vCores Allocation | 
    | ---------------|----------------|----------------|
...
If running in standalone, Lite or Minimal mode, the GlusterFS Network strorage services will not be provisioned.

If the network service is provisioned, the storage can be allocated automtically in Kubernetes using the "gluster-heketi" storage class in the persistent volume claim.

!!! tip "All storage is allocated thinly, so you can over allocate without issue, as long as you don't intend to actually use all the space. Overallocating is useful, in order to give you more volumes of varying sizes to play with, when deploying Kubernetes workloads."
...
In the example, KX.AS.CODE was started without the desktop, to re-enable it, execute the following script.
...
!!! warning "Troubleshooting"
    ...
    - I am getting warnings that my Dockerhub download limit is used up. You have 2 choices, either wait for 6 hours for the restriction to be lifted, or sign up to a free docker account, which increases your free limit. Ensure you comply with any licensing limitations.
    - I want to access my VM via SSH externally, but this is ot working. First of all, check you are using the correct port. For KX-Main1, the port is mapped to `localhost:2230`. If it still does not work, log into the workstation and check that enp0s8 correctly go an IP address. In VirtualBox, the standard IP that is setup is 10.100.76.200. If this is not configured, try rebooting the VM. If that does not help, try re-provisioning the environment.
