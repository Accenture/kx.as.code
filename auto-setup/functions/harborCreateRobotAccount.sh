harborCreateRobotAccount() {
    ...
    if [[ -z ${robotAccount} ]]; then
        ...
        pushPassword "harbor-${harborRobotUser}-robot-password" "$(cat /usr/share/kx.as.code/.config/.robot.cred)"
        pushPassword "harbor-${harborRobotUser}-robot-username" "${harborRobotUser}"
