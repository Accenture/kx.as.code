checkApplicationInstalled() {
  ...
  export appComponentName=${1}
  export appComponentInstallationFolder=${2}

  # Define component install directory
  local installComponentDirectory=${autoSetupHome}/${appComponentInstallationFolder}/${appComponentName}
  ...
  if [[ "${http_code}" == "404" ]]; then
    >&2 log_debug "Application \"${appComponentName}\" is not installed. Will skip next steps that relies on this application to be available"
    false
    return
  else
