...
if [[ -n ${mongodbUserPasswordExists} ]]; then
    ...
    export mongodbUserPassword=$(managedPassword "rocketchat-mongodb-user-password")
...
if [[ -n ${mongodbRootPasswordExists} ]]; then
    ...
    export mongodbRootPassword=$(managedPassword "rocketchat-mongodb-root-password")
