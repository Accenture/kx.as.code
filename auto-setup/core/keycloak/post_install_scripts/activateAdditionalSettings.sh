...
kubectl -n ${namespace} exec ${kcPod} --container ${kcContainer} -- /opt/jboss/keycloak/bin/kcadm.sh config credentials --server http://localhost:8080/auth --realm master --user admin --password ${keycloakAdminPassword} --client admin-cli
...
docker run \
    ...
    -e KEYCLOAK_PASSWORD=${keycloakAdminPassword} \
