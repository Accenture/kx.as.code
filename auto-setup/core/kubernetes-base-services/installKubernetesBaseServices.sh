...
if [[ ! ${kubeAdminStatus} ]] || [[ "${kubeOrchestrator}" == "k3s" ]]; then
    if [[ "${kubeOrchestrator}" == "k8s" ]]; then
        ...
        mkdir -p /root/.kube
        log_debug "INSTALL_K3S_VERSION=${k3sVersion} INSTALL_K3S_EXEC=\"--disable servicelb --disable traefik --flannel-backend=none --disable-network-policy --cluster-cidr=10.20.76.0/16 --cluster-init --advertise-address ${mainIpAddress} --kube-apiserver-arg default-not-ready-toleration-seconds=10\" bash ${installationWorkspace}/k3s-install.sh"
