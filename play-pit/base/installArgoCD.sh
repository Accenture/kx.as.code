...
for i in {1..10}; do
    ...
    if [[ -n $RESPONSE ]]; then
        echo "Added certificate to ArgoCD OK. Exiting loop"
        break
        sleep 5
    fi
done
...
for i in {1..10}; do
    ...
    if [[ -n $RESPONSE ]]; then
        echo "Added KX.AS.CODE Docs Repository to ArgoCD OK. Exiting loop"
        break
        sleep 5
    fi
done
...
for i in {1..10}; do
    ...
    if [[ -n $RESPONSE ]]; then
        echo "Added KX.AS.CODE Docs App to ArgoCD OK. Exiting loop"
        break
        sleep 5
    fi
done
...
for i in {1..10}; do
    ...
    if [[ -n $RESPONSE ]]; then
        echo "Added KX.AS.CODE TechRadar App to ArgoCD OK. Exiting loop"
        break
        sleep 5
    fi
done
