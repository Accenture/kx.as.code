...
do
  ...
  if [[ ! -z "$RESPONSE" ]]; then
    echo "Added Jira Repository to ArgoCD OK. Exiting loop"; break
    sleep 5
  fi
done
...
do
  ...
  if [[ ! -z "$RESPONSE" ]]; then
    echo "Added Jira App to ArgoCD OK. Exiting loop"; break
    sleep 5
  fi
done
...
do
  ...
  if [[ ! -z "$RESPONSE" ]]; then
    echo "Added Confluence App to ArgoCD OK. Exiting loop"; break
    sleep 5
  fi
done
