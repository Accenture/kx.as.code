...
if [[ -f ${installationWorkspace}/vagrant.json ]] && [[ $CALLER != "systemd"   ]]; then
        ...
        exit
elif [[ -f ${installationWorkspace}/vagrant.json ]] && [[ $CALLER == "systemd"   ]]; then
...
(   
    ...
    if [[ -f ${installationWorkspace}/vagrant.json.previous ]]; then
        ...
        echo -e "###############################################################################" | tee -a ${summmaryLogFile}
        echo -e "" | tee -a ${summmaryLogFile} | tee -a ${installationWorkspace}/vagrant_systemd_k8s_initialization_steps.log
    else
    ...
    while read script; do
        ...
        if [[ (! -f ${installationWorkspace}/vagrant.json.previous) || (-f ${installationWorkspace}/vagrant.json.previous && -n $DIFF)      ]]; then
